...
const dbPath = '/mnt/g/ai/MkwTrackEditor/public/data/MarioKartWii/Race/Course/course-asset-db.json';
const outputPath = '/mnt/g/ai/MkwTrackEditor/public/data/MarioKartWii/Race/Course/ExtractedAssets.u8';
...
function buildU8(entries) {
  ...
  const pushBytes = (value) => { for (const byte of value) out.push(byte); };
  const pad = (alignment, fill = 0) => { while (out.length % alignment !== 0) pushU8(fill); };
...
const preferredAssets = [...db.assets]
  .filter((asset) => asset.kind === 'object' || asset.kind === 'sharedObject')
  .sort((a, b) => Number(b.source === 'sharedObjectDir') - Number(a.source === 'sharedObjectDir'));

const selectedAssets = new Map();
for (const asset of preferredAssets) {
  const key = asset.baseName.toLowerCase();
  if (!selectedAssets.has(key)) selectedAssets.set(key, asset);
}

const trackEntryCache = new Map();
const entries = [];
for (const asset of selectedAssets.values()) {
  let data;
  if (asset.source === 'sharedObjectDir') {
    data = new Uint8Array(readFileSync(path.join(courseRoot, 'Object', asset.baseName)));
  } else {
    const trackPath = path.join(courseRoot, asset.trackFile);
    let parsed = trackEntryCache.get(trackPath);
    if (!parsed) {
      parsed = parseU8(decodeYaz0(new Uint8Array(readFileSync(trackPath))));
      trackEntryCache.set(trackPath, parsed);
    }
    const match = parsed.find((entry) => entry.path.toLowerCase() === asset.path.toLowerCase());
    if (!match?.data) throw new Error(`Missing asset ${asset.path} in ${asset.trackFile}`);
    data = match.data;
  }
  entries.push({ path: `Extracted/${asset.baseName}`, data });
}

mkdirSync(path.dirname(outputPath), { recursive: true });
writeFileSync(outputPath, buildU8(entries));
console.log(`Wrote ${entries.length} extracted assets to ${outputPath}`);
